...
public class CassandraOutputMeta extends BaseTransformMeta implements ITransformMeta<CassandraOutput, CassandraOutputData> {
  ...
  public void check( List<ICheckResult> remarks, PipelineMeta transMeta, TransformMeta stepMeta, IRowMeta prev,
    ...
    if ( ( prev == null ) || ( prev.size() == 0 ) ) {
      ...
      cr = new CheckResult( CheckResult.TYPE_RESULT_OK, "Transform is connected to previous one, receiving " + prev.size() //$NON-NLS-1$
    ...
    if ( input.length > 0 ) {
      cr = new CheckResult( CheckResult.TYPE_RESULT_OK, "Transform is receiving info from other steps.", stepMeta ); //$NON-NLS-1$
      remarks.add( cr );
    } else {
  ...
  @Override
  public ITransformMeta getStep( TransformMeta stepMeta, ITransformData stepDataInterface, int copyNr,
